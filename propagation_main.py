...
for doi in dois:
    print doi
    leaning_loader.load_tags_from_file(doi_dir +doi)
    tags = leaning_loader.tags
    # output_dir = './data/roi_data/'

    graph_wrapper.load_from_file(is_directed=True, file_path=output_dir+'uniform_sample_p_{0}.txt'.format(sample_size))
